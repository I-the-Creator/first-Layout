...
function doMagic() {
    ...
    btn.onclick = function () {
        ...
        document.querySelector('ul').classList.toggle('menu-clicked'); // вызов без объявления переенной, добавление к ul класса 'menu-clicked'
        var lis = document.querySelectorAll('.menu li'); // обращение ко всем li внутри  класса 'menu' и создание массива из них
        for(let i = 0; i < lis.length; i++) {
        // lis[i].classList.add('red');   //  чтение классов(стандартный метод) у каждого li и добавление нового класса
...
function something () {
    var btn = document.getElementById('btn');   // обращение только по id
    btn.onclick = function () {
